...
class TfPoseEstimator:
    ...
    @staticmethod
    def image_draw_humans(npimg, humans, imgcopy=False):
    ...
    @staticmethod
    def draw_humans(model, npimg, humans, imgcopy=False):
        global count, pose_state, out_angle  # global使其成为全局变量
        ...
        flat_x = [0.0 for i in range(18)]  # 存储关键点x坐标
        flat_y = [0.0 for i in range(18)]  # 存储关键点y坐标

        # 从json中读取上一帧数据
        filename = 'output.json'
        with open(filename) as f:
            flat_before = json.load(f)
        print('前关键点', flat_before)

        for human in humans:
            # draw point
            for i in range(common.CocoPart.Background.value):
                if i not in human.body_parts.keys():  # 若未检测出i对应的点
                    flat[i * 2] = flat_before[i * 2]  # 若centre为0用上一帧数据代替
                    flat[i * 2 + 1] = flat_before[i * 2 + 1]

                    flat_x[i] = flat_before[i * 2]  # 将未检测到的点的x y 用上一帧数据替代
                    flat_y[i] = flat_before[i * 2 + 1]
                    print("miss", i)  # 打印出此次未检测出的点
                    continue
        ...
        def angle(v1, v2):
            """
                Args:输出向量夹角
                v1: 向量A=【a,b,c,d】 (a,b)->(c,d)
                v2: 向量B=【a,b,c,d】 (a,b)->(c,d)
            Returns:angle
            """
            dx1 = v1[2] - v1[0]
            dy1 = v1[3] - v1[1]
        ...
        def PullUp_count(state, cou):
            """
            计算引体向上个数 用颈部与耳朵的中间值与手部比较得到
            state：用0，1表示up和down
            cou：count的传递参数
            """
            global pullup_judge, wrong_pose_flag

            A = [flat[8], flat[9], flat[6], flat[7]]  # A为右手至右肘向量
            B = [flat[4], flat[5], flat[6], flat[7]]  # B为右肩至右肘向量
            angle1 = angle(A, B)  # AB夹角
            global_var.set_value('angle', angle1)  # 左右侧最大夹角写入全局变量

            if max(flat[9], flat[15]) < max(flat[7], flat[13]):  # 手部坐标在肘部上方（悬挂姿势） 消除引体向上前的准备动作的误判
                if (flat[3] + min(flat[33], flat[35])) / 2 < min(flat[9], flat[15]):  # 用颈部与耳朵的中间值与手部比较来判断是否拉上去
                    if state == 0:  # 若PoseState的状态由0变为1 则视为一次引体向上
                        cou = cou + 1  # 引体向上数量加一
                        if pullup_judge == 0:  # 若pullup_judge未置1 则判断为手臂直
                            wrong_pose_flag = 1
                    state = 1  # 将PoseState状态置为1
                    print("up")
                else:
                    state = 0  # 将PoseState状态置为0
                    print("down")
                    # 判断down状态时有没有手臂伸直
                    if angle1 > 150:
                        pullup_judge = 1

                print("下颌节点", (flat[3] + min(flat[33], flat[35])) / 2)
                print("左右手点", flat[9], flat[15])
            return state, cou

        def PullUp_count1(state, cou, angle1_set):
            """
            计算引体向上个数 用大小臂夹角得到
            state：用0，1表示up和down
            cou：count的传递参数
            ...
            """
            if max(flat[9], flat[15]) < max(flat[7], flat[13]):  # 手部坐标在肘部上方（悬挂姿势） 消除引体向上前的准备动作的误判
                # 输出右肘部与右肩部的夹角（以肘部节点画水平线，肘部与肩部连线向量和水平线夹角）flat[4] flat[6]分别为肘部与肩部x坐标 5 7为y坐标
                angle1 = math.atan2(flat[7] - flat[5], abs(flat[4] - flat[6]))  # atan2 输出y/x的反正切
                # abs（）转化水平线为绝对值防止学生正反面对镜头产生影响
                angle1 = int(angle1 * 180 / math.pi)  # 弧度转化为角度
                print('大小臂夹角', angle1)
                if angle1 > angle1_set:  # 用大小臂夹角比较来判断是否拉上去
                    if state == 0:  # 若PoseState的状态由0变为1 则视为一次引体向上
                        cou = cou + 1  # 引体向上数量加一
                    state = 1  # 将PoseState状态置为1
                    print("up")
        ...
        def PushUP_judge(angle_now, angle_stand):
            global wrong_pose_flag
            if angle_now < angle_stand:
                wrong_pose_flag = 1

        def PushUp_count(state, cou, angle1_set):
            """
            计算俯卧撑个数 用大小臂夹角得到
            state：用0，1表示up和down
            cou：count的传递参数
            angle1_set：下去时夹角设定值
            """
            A = flat[10:14]  # A为左肩至做肘部向量
            B = [flat[14], flat[15], flat[12], flat[13]]  # B为小臂向量
            angle_left = angle(A, B)  # AB夹角
            print("左侧大小臂夹角", angle_left)

            A = flat[4:8]  # A为右肩至做肘部向量
            B = [flat[8], flat[9], flat[6], flat[7]]  # B为小臂向量
            angle_right = angle(A, B)  # AB夹角
            print("右侧大小臂夹角", angle_right)

            A = [flat[2], flat[3], flat[22], flat[23]]  # A为颈部朝向腰部向量
            B = [flat[24], flat[25], flat[22], flat[23]]  # B大腿向量
            angle_left_body = angle(A, B)  # AB夹角
            print("左侧躯干夹角", angle_left_body)

            A = [flat[2], flat[3], flat[16], flat[17]]  # A为颈部朝向腰部向量
            B = [flat[18], flat[19], flat[16], flat[17]]  # B大腿向量
            angle_right_body = angle(A, B)  # AB夹角
            print("右侧躯干夹角", angle_right_body)

            out_angle.append(angle_right_body)  # 末尾添加angle_right_body
            print("角度数据", out_angle)

            global_var.set_value('angle', angle_right_body)  # 左右侧最大夹角写入全局变量
            # 用来判断生体是否上凸或者下凹

            PushUP_judge(angle_right_body, 165)  # 判断动作是否标准

            if flat[0] > flat[2]:  # 如果鼻子坐标大于颈部坐标 认定为头朝右方  此时用右臂进行判断
                print('身体朝右')
                if angle_right > angle1_set:  # 用大小臂夹角比较来判断是否按下去
                    if state == 0:  # 若PoseState的状态由0变为1 则视为成功
                        cou = cou + 1  # 数量加一
                    state = 1  # 将PoseState状态置为1
                    print("up")
                else:
                ...
                print("身体朝左")
                if angle_left > angle1_set:  # 用大小臂夹角比较来判断是否按下去
                    if state == 0:  # 若PoseState的状态由0变为1 则视为成功
                        cou = cou + 1  # 数量加一
                    state = 1  # 将PoseState状态置为1
                    print("up")
        ...
        def SitUp_count(state, cou, angle1_set):
            """
            计算仰卧起坐个数 用躯干与大腿夹角得到
            state：用0，1表示up和down
            cou：count的传递参数
            angle1_set：坐起时夹角设定值
            """
            A = [flat[2], flat[3], flat[16], flat[17]]  # A为颈部至做右胯向量
            B = [flat[18], flat[19], flat[16], flat[17]]  # B为右大腿向量
            angle1 = angle(A, B)  # AB夹角
            print("右侧身体夹角", angle1)
            if angle1 < angle1_set:  # 用夹角比较来判断是否坐起
                if state == 0:  # 若PoseState的状态由0变为1 则视为成功
                    cou = cou + 1  # 数量加一
                state = 1  # 将PoseState状态置为1
                print("up")
        ...
        # pose_state, count = PullUp_count(pose_state, count)  # PullUp计数  比较节点
        # pose_state, count = PullUp_count1(pose_state, count, 35)  # PullUp计数 夹角判断
        # pose_state, count = PushUp_count(pose_state, count, 130)  # PushUp计数 夹角判断
        # pose_state, count = SitUp_count(pose_state, count, 90)  # SitUp计数 夹角判断

        def model_choice_count(model):
            """
            选择不同体测项目并用相应标准计数
            """
            global pose_state, count
            if model == 1:
                pose_state, count = PushUp_count(pose_state, count, 130)  # model=1  PushUp计数 夹角判断
            if model == 2:
                pose_state, count = PullUp_count(pose_state, count)  # model=2 PullUp计数  比较节点
            if model == 3:
                pose_state, count = SitUp_count(pose_state, count, 90)  # model=3 SitUp计数 夹角判断

        model_choice_count(model)  # 根据GUI界面选择的model选择体测项目并计数


        global_var.set_value('count', count)  # 写入全局变量  将计数值传进run_video(1)并在结果界面显示
        global_var.set_value('judge', wrong_pose_flag)  # 将wrong_pose_flag写入全局变量

        # output list 以json格式
        filename = 'output.json'
        with open(filename, 'w') as file_obj:
            json.dump(flat, file_obj)
        print('关键点 ', flat)  # 打印出关键点数据
